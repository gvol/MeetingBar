...
final class PomodoroManager {
    /// Set after StatusBarItemController is created. Gives access to events + window presentation.
    ...
    private func handlePhaseEnd() {
        ...
        timer = nil
        refreshUI()
        guard let statusBar = statusBar, let type = type else { return }

        switch phase {
        case .work:
            statusBar.openPomodoroNotification(
                manager: self,
